...
from ..layers.mobilenet_conv_layer import (
    Conv2dNormActivation,
    SqueezeExcitation as SElayer,
)
from ..misc._api import register_model, Weights, WeightsEnum
from ..misc._utils import _make_divisible, handle_legacy_interface
...
def _mobilenet_v3(
    ...
    **kwargs: Any,
    ...
    if weights is not None and kwargs.get("num_classes"):
        print("Loading pretrained weights...")
        if weights.meta["categories"] != kwargs["num_classes"]:
            loaded_state_dict = weights.get_state_dict(progress=progress)
            state_dict = {
                k: v for k, v in loaded_state_dict.items() if "classifier" not in k
            }
...
from ..misc._utils import _ModelURLs
