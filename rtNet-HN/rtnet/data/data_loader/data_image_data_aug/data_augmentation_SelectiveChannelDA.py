...
from rtnet.data.data_loader.data_image_data_aug.custom_transforms import (
    SelectiveChannelBrightnessTransform,
)
from rtnet.data.data_loader.data_image_data_aug.spatial_transform_v2 import (
    SpatialTransformV2,
    MirrorTransform,
)
...
from rtnet.data.data_loader.data_image_data_aug.custom_transforms import (
    ...
    SelectiveChannelGammaTransform,
    SelectiveChannelGaussianBlurTransform,
    SelectiveChannelSimulateLowResolutionTransform,
    SelectiveChannelBrightnessMultiplicativeTransform,
    SelectiveChannelGaussianNoiseTransform,
    SelectiveChannelContrastAugmentationTransform,
)
from rtnet.data.data_loader.data_image_data_aug.default_data_augmentation import (
    default_3D_augmentation_params,
)
from rtnet.data.data_loader.data_image_data_aug.downsampling import (
    DownsampleSegForDSTransform3,
    DownsampleSegForDSTransform2,
)
from rtnet.data.data_loader.data_image_data_aug.pyramid_augmentations import (
...
def get_selective_channel_augmentation(
    dataloader_train,
    dataloader_val,
    patch_size,
    ...
    ## move dist, disc_seg, disc_att from data to seg, and apply augmentation in seg mode.
    # tr_transforms.append(MoveAuxData(key_origin='data', key_target='seg'))
    ...
    tr_transforms.append(
        SpatialTransformV2(
            ...
            scheme=params.get("normalization_schemes"),
            dummy_2d=params.get("dummy_2D"),
        )
    )
    ...
    idx_aug = []
    for i in params.get("normalization_schemes"):
        if params.get("normalization_schemes")[i] not in ["support", "heatmap"]:
            idx_aug.append(i)

    # we need to put the color augmentations after the dummy 2d part (if applicable). Otherwise the overloaded color
    # channel gets in the way
    tr_transforms.append(
        SelectiveChannelGaussianNoiseTransform(p_per_sample=0.1, idx_aug=idx_aug)
    )
    tr_transforms.append(
        SelectiveChannelGaussianBlurTransform(
            (0.5, 1.0),
    ...
    tr_transforms.append(
        SelectiveChannelBrightnessMultiplicativeTransform(
            multiplier_range=(0.75, 1.25), p_per_sample=0.15, idx_aug=idx_aug
        )
    )
    ...
    tr_transforms.append(
        SelectiveChannelContrastAugmentationTransform(
            p_per_sample=0.15, idx_aug=idx_aug
        )
    )
    tr_transforms.append(
        SelectiveChannelSimulateLowResolutionTransform(
