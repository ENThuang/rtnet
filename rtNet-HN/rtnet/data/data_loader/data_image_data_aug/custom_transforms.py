...
from batchgenerators.augmentations.noise_augmentations import (
        ...
        for b in range(len(data_dict[self.data_key])):
            if np.random.uniform() < self.p_per_sample:
                if self.idx_aug is not None and len(self.idx_aug) > 0:
                    ...
                    data_dict[self.data_key][b] = augment_brightness_additive(
                        data_dict[self.data_key][b],
                        self.mu,
                        self.sigma,
                        self.per_channel,
...
class SelectiveChannelGammaTransform(AbstractTransform):
    ...
    def __call__(self, **data_dict):
        for b in range(len(data_dict[self.data_key])):
            if np.random.uniform() < self.p_per_sample:
                if self.idx_aug is not None and len(self.idx_aug) > 0:
                    data_dict[self.data_key][b][self.idx_aug] = augment_gamma(
                        data_dict[self.data_key][b][self.idx_aug],
                        self.gamma_range,
                        self.invert_image,
                        per_channel=self.per_channel,
