...
def unpack_dataset(folder, threads=cfg.default_num_threads, key="data"):
    """
...
def pack_dataset(folder, threads=cfg.default_num_threads, key="data"):
    p = Pool(threads)
...
class DataLoader3D(SlimDataLoaderBase):
    def __init__(
        ...
        pad_sides=None,
        oversample_foreground_class=None,
    ):
        ...
        self.oversample_foreground_class = oversample_foreground_class
        ...
        if self.patch_size is not None:
            self.need_to_pad = (
                np.array(patch_size) - np.array(final_patch_size)
            ).astype(int)
        if pad_sides is not None and self.patch_size is not None:
    ...
    def generate_train_batch(self):
        ...
        if self.patch_size is None and self.batch_size == 1:
            case_properties = []
            for j, i in enumerate(selected_keys):
                ...
                data_shape = (
                    self.batch_size,
                    case_all_data.shape[0] - 1,
                    *case_all_data.shape[1:],
                )
                seg_shape = (self.batch_size, num_seg, *case_all_data.shape[1:])
            ...
            for j, i in enumerate(selected_keys):
                ...
                if self.has_prev_stage:
                    ...
                    assert all(
                        [
                            i == j
                            for i, j in zip(
                                seg_from_previous_stage.shape[1:],
                                case_all_data.shape[1:],
                            )
                        ]
                    ), (
                ...
                if not force_fg:
                    ...
                    if "class_locations" not in properties.keys():
                        raise RuntimeError(
                            "Please rerun the preprocessing with the newest version of rtNet!"
                        )
                    ...
                    if len(foreground_classes) == 0:
                        ...
                        if self.oversample_foreground_class is None:
                            selected_class = np.random.choice(foreground_classes)
                        else:
                            selected_class = np.random.choice(
                                list(self.oversample_foreground_class)
                            )
